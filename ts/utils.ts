 ...
 */
function loadCards(cardsEl: HTMLElement, workContainerEl: HTMLElement
) {
    for (let i = 0; i < cardsEl.children.length; i++) {
        const cardEl = cardsEl.children[i];
        if (cardEl) {
            (cardEl as HTMLElement).style.display = "flex";
        }
 ...
 */
function hideCards(className: string, cardsEl: HTMLElement, workEl: HTMLElement) {
